$(document).ready(function () {
  ...
  $("#hover Profile").hover(
    function () {
      ...
      $(this)
        .find("img")
        .css({ transform: "scale(1.1)", filter: "brightness(1.2)" });
    },
    function () {
      // mouseleave
      ...
      $(this)
        .find("img")
        .css({ transform: "scale(1)", filter: "brightness(1)" });
    }
  );
  ...
  $("#alumniForm").submit(function (event) {
    ...
    if (name && year && photo) {
      ...
      reader.onload = function (e) {
        const newRow = ` 
<tr> <td class="name">${name}</td> <td class="year">${year}</td> <td><img src="${e.target.result}" alt="Photo $(name)" class="alumni-
...
<button class="delete">Hapus</button>
  ...
  $("#alumniTable")
    .on("mouseenter", "img", function () {
      $(this).css({ transform: "scale(1.1)", filter: "brightness(1.2)" });
    })
